...
//    对 BypassMergeSortShuffleHandle 进行的判断 => 在不需要map端聚合、partition数量小于200的情况返回BypassMergeSortShuffleHandle对象
...
//    对 SerializedShuffleHandle 进行的判断 => 在不需要map端聚合、partition数量小于16777216，Serializer支持relocation的情况下
