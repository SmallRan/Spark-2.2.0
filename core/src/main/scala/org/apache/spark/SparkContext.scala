...
class SparkContext(config: SparkConf) extends Logging {
   ...
   * @Date: 2022/5/31
   * @Description:
   * SparkContext 的初始化主要从这里开始
   */
  try {
    _conf = config.clone()
    _conf.validateSettings()

//    判断是否有 spark.master 参数
     ...
     * @Date: 2022/5/31
     * @Description:
     *  如果spark.eventLog.enabled为true,默认是false，如果设置成true则记录Spark事件的根目录。
     *  在这个根目录中， Spark为每个应用程序创建一个子目录，并在该目录中记录特定于应用程序的事件。
     *  可以设置它成 hdfs 目录，以便历史记录服务器可以读取历史文件。
     */
    _eventLogDir =
     ...
     * @Date: 2022/5/31
     * @Description: 第一步
     * 开始创建 Spark 执行环境 SparkEnv，
     * 除此之外还初始化了各种 Manager 对象；例如SortShuffleManager、useLegacyMemoryManager、broadcastManager等等
     */
...
//    每个 Executor 进程使用的内存量,默认是1G, 或提交参数时指定：--executor-memory
     ...
     * @Author: Small_Ran
     * @Date: 2022/5/31
     * @Description: 第四步 创建心跳接收器
     *  1、我们需要在 createTaskScheduler 之前注册 HeartbeatReceiver ，因为 Executor 将在构造函数中检索 HeartbeatReceiver
     *  2、创建一个HeartbeatReceiver 的 RpcEndpoint 注册到 RpcEnv 中，每分钟给自己发送 ExpireDeadHosts 检测 Executor 是否存在心跳，
     *  3、如果当前时间减去最后一次心跳时间大于1分钟，就会用 CoarseGrainedSchedulerBackend 将 Executor 杀死
     *  4、Driver端心跳接收器，用于接收各个 Executor 的心跳
     */
     ...
     * @Author: Small_Ran
     * @Date: 2022/5/31
     * @Description:第五步 创建任务调度TaskScheduler
     * 如果集群管理器是standalone模式：该方法返回（StandaloneSchedulerBackend，TaskSchedulerImpl）
     * TaskSchedule 的实现类 -> TaskSchedulerImpl
     * SchedulerBackend 有两个成员变量：StandaloneAppClient（负责和 Master通讯）、driverEndpoint（负责和 Executor 通讯）
     */
     ...
     * @Author: Small_Ran
     * @Date: 2022/6/1
     * @Description: 第六步 创建和启动DAGScheduler
     *  1、内部初始化了一个：DAGSchedulerEventProcessLoop 用来处理各种事件
     ...
     * @Author: Small_Ran
     * @Date: 2022/6/1
     * @Description: 第七步 TaskScheduler的启动
     * 主要任务：backend.start()
     */
     ...
     * @Author: Small_Ran
     * @Date: 2022/7/13
     * @Description:  第八步：初始化管理器 BlockManager
     */
     ...
     * @Author: Small_Ran
     * @Date: 2022/7/13
     * @Description:  第九步：启动测量系统MetricsSystem
     */
     ...
     * @Author: Small_Ran
     * @Date: 2022/7/13
     * @Description:  第十步：创建和启动Executor分配管理器ExecutorAllocationManager
     */
     ...
     * @Author: Small_Ran
     * @Date: 2022/7/13
     * @Description:  第十一步：ContextCleaner的启动和创建
     */
    _cleaner =
     ...
     * @Author: Small_Ran
     * @Date: 2022/7/13
     * @Description:  第十三步：创建DAGSchedulerSource和BlockManagerSource
     */
    // Post init
     ...
     * @Author: Small_Ran
     * @Date: 2022/7/13
     * @Description:  第十四步：将SparkContext标记激活
     */
  ...
  def runJob[T, U: ClassTag](
     ...
     * @Author: Small_Ran
     * @Date: 2022/7/13
     * @Description: 将任务提交至 DAGscheduler 进行Job 任务的划分
     * 大概流程：
     *   1、应用程序调用 action 算子
...
//        Spark 外部资源管理器，目前支持三种（KubernetesClusterManager、YarnClusterManager、MesosClusterManager）
      case masterUrl =>
//        这里返回的是 //        这里返回的是 YarnClusterManager
