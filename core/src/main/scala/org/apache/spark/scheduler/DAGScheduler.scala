...
//      如果有就直接返回
      case Some(stage) =>
         ...
         * @Date: 2022/7/13
         * @Description:
         * 1、如果还有 ShuffleDependency 没有构建 ShuffleMapStage 则创建一个
         * 2、如果这个stage没有构建，先把这个stage的所有父stage都构建出来，然后再次构建当前stage
         */
      case None =>
        // Create stages for all missing ancestor shuffle dependencies.
//        给当前 stage 的所有 父stage 创建 ShuffleMapStage
...
//        为指定的 ShuffleDependency 创建一个 ShuffleMapStage
...
//                若为窄依赖，那就属于同一个stage；并将该 RDD 的父 RDD 放入堆中；以能够在下面的while中继续向上visit，直至遍历了整个DAG图
...
//      调用上面的方法
  ...
  def submitJob[T, U](
     ...
     * @Author: Small_Ran
     * @Date: 2022/7/13
     * @Description:  通过循环事务处理提交 Job
     * 1、eventProcessLoop 就是当初 DAGScheduler 在初始化的时候，创建的一个 DAGSchedulerEventProcessLoop
     * 2、这个组件主要负责：任务的提交执行，把 JobSubmitted 这个消息，放入了 eventQueue 队列中
     */
  ...
  def runJob[T, U](
     ...
     * @Author: Small_Ran
     * @Date: 2022/7/13
     * @param rdd   要在其上运行任务的参数RDD目标RDD
     * @param func  在RDD的每个分区上运行的函数
     * @param partitions  要运行的分区的集；某些作业可能不希望在目标RDD的所有分区上进行计算，例如，对于 first() 之类的操作
     * @param callSite  在用户程序中调用此作业的位置
     * @param resultHandler 回调函数，以将每个分区结果传递给Xxx
     * @param properties  要附加到此作业的scheduler属性，例如fair scheduler pool name
     * @Description:  提交 Job
     */
   ...
   * @Author: Small_Ran
   * @Date: 2022/7/13
   * @Description:  DAGScheduler 调度的核心人口
   *  大致思路：
   *      1、RDD DAG 划分 Stages：Stage的划分是从最后一个Stage开始逆推的，每遇到一个宽依赖处，就分裂成另外一个Stage，依此类推直到Stage划分完毕为止；
   *      2、只有最后一个Stage的类型是ResultStage类型
   *注：Dataset、DataFrame、sparkSession.sql("select ...")经过catalyst代码解析会将代码转化为RDD
   */
...
//    第一步：创建 ResultStage（RDD 链条中最后一个 RDD，也是 sc.runJOb 执行的 RDD；换句话来说就是该 RDD 也一定会调用了一个 Action 算子）
...
//    第四步： 使用 submitStage 提交 finalStage
  ...
  /** Submits stage, but first recursively submits any missing parents. */
     ...
     * @Date: 2022/7/13
     * @Description:
     * 1、stage是 ResultStage，目标就是提交最后一个Stage
...
//            首先创建新的 Stage 是从最后一个 RDD开始，所以一直递归最终是从创建最后一个 Stage 开始提交
  ...
  /** Called when stage's parents are available and we can now do its task. */
     ...
     * @param stage
     * @param jobId
     * @Description: 提交一个stage的Task来执行
     * 创建 Task，其中 Task 数量与 Partition 数量相同
     * 第一步：得到 RDD 中需要计算的 partition
     * 第二步：序列化 task 的 binary
     * 第三步：为每个需要计算的 partiton 生成一个 task
     * 第四步：提交tasks
     */
  private def submitMissingTasks(stage: Stage, jobId: Int) {
     ...
     * @Author: Small_Ran
     * @Date: 2022/7/13
     * @Description: 第一步：得到 RDD 中需要计算的 partition
     *  1、首先得到 RDD 中需要计算的 partition，对于 Shuffle 类型的 stage，需要判断 stage 中是否缓存了该结果；
     *  2、对于 Result 类型的 Final Stage，则判断计算 Job 中该 partition 是否已经计算完成；
     *  3、stage 中某个 task 执行失败其他执行成功地时候就需要找出这个失败的 task 对应要计算的 partition 而不是要计算所有 partition；
     */
     ...
     * @Author: Small_Ran
     * @Date: 2022/7/13
     * @Description: 第二步：序列化 task 的 binary
     *  Executor 可以通过广播变量得到它，每个 task 运行的时候首先会反序列化；
     */
...
//    按照不同的依赖创建 Task
     ...
     * @Author: Small_Ran
     * @Date: 2022/7/13
     * @Description: 第二步：为每个需要计算的 partiton 生成一个 task
     *   ShuffleMapStage 对应的 task 全是 ShuffleMapTask; ResultStage 对应的全是 ResultTask。
     *   task 继承 Serializable，要确保 task 是可序列化的。
     *
     *ShuffleMapTask 和 ResultTask 相同点：
     *   1、这两种 Task 都是在 RDD 的分区上执行的。
     *   2、两种 Task 都需要调用父 RDD 的 iterator 方法来获取父 RDD 对应分区的数据。
     *   3、这些数据可以直接来自于数据源，也可以直接来自于上一个 ShuffleMapTask 执行的结果。
     *   4、当一个 Stage 中所有分区的 Task 都执行完毕，这个 Stage 才算执行完毕。
     *
     *ShuffleMapTask 和 ResultTask 不同点：
     *   1、ResultTask 获取父 RDD 分区数据之后，把分区数据作为参数输入到 action 函数中，最终计算出特定的结果返回给 driver。
     *   2、ShuffleMapTask 获取父 RDD 分区数据之后，把分区数据作为参数传入分区函数，最终形成新的 RDD 中的分区数据，保存在各个 Executor节点中，并将分区数据信息 MapStatus 返回给 driver。
     */
...
//          partitionsToCompute 这个集合，就是当前stage的最后一个RDD的分区的信息
//          如果是 ShuffleMapStage，则把这个stage中的最后一个RDD的每个分区的信息封装成一个ShuffleMapTask
          partitionsToCompute.map { id =>
...
//      提交任务至 TaskScheduler,taskScheduler 的具体类型是：TaskSchedulerImpl
//      tasks 本身是一个 Seq, 转变成给一个 数组，然后封装成一个 TaskSet 对象
 ...
 * @Date: 2022/6/1
 * @Description:
 *   1、DAGSchedulerEventProcessLoop 继承了 EventLoop 抽象类（其实 EventLoop 也是泛型类，这里泛型类型为 DAGSchedulerEvent ），
 *      并在构造函数中传递了 DAGScheduler 类对象;
 *   2、对外提供了 DAGSchedulerEvent 接收 DAGSchedulerEvent 事件，并将接收到 DAGSchedulerEvent 事件存储到队列中;
 *   3、在内部阻塞死循环方式去从队列中获取 DAGSchedulerEvent 事件，
 *      获取到后并处理它[调用 onReceive (event: DAGSchedulerEvent)方法];
 *   4、onReceive 方式内部调用了私有方法 doOnReceive(event)，
 *      在 doOnReceive 方法中会根据event类型不同去调用 dagScheduler 的不同 handleXxx 方法（真正事件处理最后还归结于 DAGScheduler 中）;
 */
private[scheduler] class DAGSchedulerEventProcessLoop(dagScheduler: DAGScheduler)
  ...
  override def onReceive(event: DAGSchedulerEvent): Unit = {
    ...
    try {
//      会根据 event 类型不同去调用 dagScheduler 的不同 handleXxx 方法
      doOnReceive(event)
    } finally {
      timerContext.stop()
    }
