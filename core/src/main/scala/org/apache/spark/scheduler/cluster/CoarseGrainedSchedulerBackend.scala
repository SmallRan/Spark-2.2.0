...
//        当前这个 Task 的序列化数据如果超过规定的 RpcMessage 的最大大小
...
//          实际上：发送 LaunchTask 消息给：CoarseGrainedExecutorBackend
//          逻辑上：发送 LaunchTask 消息给: Executor
...
//    给 DriverEndpoint 发送 ReviveOffers 消息
